...
class _CondaAPI(QObject):
    ...
    def _set_environment_variables(self, prefix=None, no_default_python=False):
        ...
        # TODO: this is meant to short-circuit failure, such that unsatisfiable errors for rstudio happen faster.
        ...
        # environ_copy['CONDA_UNSATISFIABLE_HINTS'] = "False"
                ...
                for dir in self.pkgs_dirs:
                    ...
                    if fpath in self._app_info_cache:
                        if self._app_info_cache[fpath]:
                            ...
                            # do nothing if the cache entry is None.  It means that we've seen it
...
#        return ordered_sources
    ...
    def pip_list(self, name=None, prefix=None, abspath=True):
        ...
        try:
            sp_dir = subprocess.check_output([pyth_command, '-c', "import site; print([_ for _ in site.getsitepackages() if _.endswith('site-packages')][0])"])
            sp_dir = sp_dir.strip()
