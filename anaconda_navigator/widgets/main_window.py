...
#        self.check_dependencies_before_install(worker, worker.output, error)
    ...
    def conda_application_action(
        ...
        if non_conda:
            if action == C.APPLICATION_INSTALL:
                ...
                self.remove_external_app(package_name)
            return
    ...
    def install_external_app(self, app_name):
        ...
        worker = external_apps[app_name](
            config=self.config,
            process_api=self.api._process_api).install(password=password)
        worker.sig_partial.connect(functools.partial(
            self._partial_external_app_ready, app_name=app_name))
        worker.sig_finished.connect(self._output_external_app_ready)

    def remove_external_app(self, app_name):
        """Removing external app."""
        self.update_status(
            'Removing <b>{}<b>'.format(app_name),
            '',
            value=0,
            max_value=0,
        )
        ...
        if not cancelled:
            worker = external_apps[app_name](
                config=self.config,
                process_api=self.api._process_api).remove(password=password)
            worker.sig_partial.connect(functools.partial(
                self._partial_external_app_ready, app_name=app_name))
            worker.sig_finished.connect(self._output_external_app_ready)
